...
function isSafePlanJson(value: unknown, seen = new Set<object>()): boolean {
  ...
  if (Array.isArray(value))
    return value.every((entry) => isSafePlanJson(entry, seen));
  const proto = Object.getPrototypeOf(value);
  if (proto !== Object.prototype && proto !== null) return false;
  return Object.entries(value as Record<string, unknown>).every(
    ([key, entry]) =>
      !UNSAFE_OBJECT_KEYS.has(key) && isSafePlanJson(entry, seen),
  );
}
...
export function normalizeConditionReferences(
  expr: string,
  priorStepIds: string[],
): string {
  ...
  for (const id of [...priorStepIds].sort((a, b) => b.length - a.length)) {
    ...
    out = out.replace(
      new RegExp(`(^|[^A-Za-z0-9_$.-])(${escaped})(?=\\.result\\b)`, "g"),
      `$1results.$2`,
    );
  }
  return out;
...
export function validateConditionSyntax(expr: string): string | null {
  ...
  if (
    /=>|;|`|\b(new|function|return|process|globalThis|require|import|eval)\b/.test(
      src,
    )
  )
    return "condition contains executable/forbidden syntax";
  if (/\.(some|every|map|filter|reduce|test)\s*\(/.test(src))
    return "condition uses an unsupported callback/regex method";
  if (/[^A-Za-z0-9_$.[\]()?'"\\\s=!<>&|,.-]/.test(src))
    return "condition contains unsupported characters";
  let depth = 0;
  let quote = "";
  for (let i = 0; i < src.length; i++) {
    ...
    if (c === "(") depth++;
      ...
      return "condition has unbalanced parentheses";
  }
  if (depth !== 0 || quote)
    return quote
      ? "condition has an unterminated string"
  ...
  const splitTopLevel = (value: string, op: "&&" | "||"): string[] => {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (q) {
        buf += c;
        if (c === "\\" && i + 1 < value.length) buf += value[++i]!;
        else if (c === q) q = "";
        continue;
      }
      if (c === "'" || c === '"') {
  ...
  const atomOk = (value: string): boolean => {
    ...
    if (new RegExp(`^Array\\.isArray\\(\\s*${path}\\s*\\)$`).test(atom))
      return true;
    if (
      new RegExp(`^${path}\\.includes\\(\\s*${literal}\\s*\\)$`, "i").test(atom)
    )
      return true;
    if (
      new RegExp(
        `^${path}\\s*(?:===|!==|==|!=|>=|<=|>|<)\\s*(?:${literal}|${path})$`,
        "i",
      ).test(atom)
    )
      return true;
  ...
  return expressionOk(src)
    ? null
    : "condition is natural language or not in the safe expression DSL";
}

const ERROR_POLICY_ACTIONS = new Set<ErrorPolicyAction>([
  "park",
  "retry",
  "terminal",
  "continue",
...
function normalizeBareErrorLiterals(expr: string): string {
  ...
  return expr
    .replace(
      /\.includes\(\s*([A-Za-z_$][A-Za-z0-9_$.-]*)\s*\)/g,
      (_m, value: string) => `.includes(${scalar(value)})`,
    )
    .replace(
      /(===|!==|==|!=|>=|<=|>|<)\s*([A-Za-z_$][A-Za-z0-9_$.-]*)(?=\s*(?:&&|\|\||\)|$))/g,
      (_m, op: string, value: string) => `${op}${scalar(value)}`,
    );
}
...
export function validateErrorPredicateSyntax(expr: string): string | null {
  ...
  const withoutStrings = normalized.replace(
    /'(?:\\.|[^'\\])*'|"(?:\\.|[^"\\])*"/g,
    "",
  );
  const identifiers =
  ...
  for (const token of identifiers) {
    ...
    if (
      !root ||
      !["kind", "code", "status", "name", "message", "data", "meta"].includes(
        root,
      )
    ) {
...
function parseErrorPolicy(raw: unknown): ErrorPolicyRule[] | undefined {
  ...
  for (const item of raw) {
    ...
    const outcome = String(
      row.do ?? row.default ?? "",
    ).trim() as ErrorPolicyAction;
    if (!ERROR_POLICY_ACTIONS.has(outcome)) continue;
    const hasRuleDefault =
      Object.prototype.hasOwnProperty.call(row, "defaultResult") ||
      Object.prototype.hasOwnProperty.call(row, "default_result");
    const shared = {
      ...(hasRuleDefault
        ? {
        ...
        : {}),
      ...((row.emitEvent ?? row.emit_event) != null
        ? { emitEvent: String(row.emitEvent ?? row.emit_event) }
        : {}),
      ...(row.emitPayload &&
      typeof row.emitPayload === "object" &&
      !Array.isArray(row.emitPayload)
        ? { emitPayload: row.emitPayload as Record<string, unknown> }
        : row.emit_payload &&
            typeof row.emit_payload === "object" &&
            !Array.isArray(row.emit_payload)
      ...
      ...(() => {
        ...
        return {
          dependencySignal: {
            ...
            ...(typeof (d.viaTool ?? d.via_tool) === "string"
              ? { viaTool: String(d.viaTool ?? d.via_tool) }
              : {}),
          },
        };
      })(),
...
export function parsePlan(raw: unknown): PlanStep[] {
  ...
  for (const r of raw) {
    ...
    const errorPolicy = parseErrorPolicy(
      o.errorPolicy ??
        o.error_policy ??
        (Array.isArray(rawOnError) ? rawOnError : undefined),
    );
    ...
    const step: PlanStep = {
      ...
      toolArguments:
        rawToolArguments &&
        typeof rawToolArguments === "object" &&
        !Array.isArray(rawToolArguments)
          ? (Object.fromEntries(
              Object.entries(rawToolArguments as Record<string, unknown>).map(
                ([name, raw]) => {
      ...
      resultMap: (() => {
        if (
          !rawResultMap ||
          typeof rawResultMap !== "object" ||
          Array.isArray(rawResultMap)
        )
          return undefined;
      ...
      condition: rawCondition
        ? normalizeConditionReferences(rawCondition, priorStepIds)
        : undefined,
      // #G2 — the two declared events this condition routes to.
      routes: (() => {
        const raw = (o.routes ?? o.route) as
        ...
        if (typeof onTrue !== "string" || typeof onFalse !== "string")
          return undefined;
      ...
      invokeInput:
        o.invokeInput &&
        typeof o.invokeInput === "object" &&
        !Array.isArray(o.invokeInput)
          ? (o.invokeInput as Record<string, unknown>)
          : o.invoke_input &&
              typeof o.invoke_input === "object" &&
              !Array.isArray(o.invoke_input)
            ? (o.invoke_input as Record<string, unknown>)
            : undefined,
      forwardLastResult:
        typeof (o.forwardLastResult ?? o.forward_last_result) === "boolean"
          ? Boolean(o.forwardLastResult ?? o.forward_last_result)
          : undefined,
      forwardResults:
        typeof (o.forwardResults ?? o.forward_results) === "boolean"
          ? Boolean(o.forwardResults ?? o.forward_results)
          : undefined,
      dependsOn: Array.isArray(dependsRaw) ? dependsRaw.map(String) : undefined,
      idempotencyKeyFrom:
        (o.idempotencyKeyFrom ?? o.idempotency_key_from) != null
          ? String(o.idempotencyKeyFrom ?? o.idempotency_key_from)
          : undefined,
      onError:
        onErrRaw === "soft" || onErrRaw === "terminal" || onErrRaw === "park"
          ? (onErrRaw as PlanStep["onError"])
          : undefined,
      errorPolicy,
      defaultResult: hasDefaultResult
        ...
        : undefined,
      timeoutS:
        typeof (o.timeoutS ?? o.timeout_s) === "number"
          ? ((o.timeoutS ?? o.timeout_s) as number)
          : undefined,
      itemsFrom:
        (o.itemsFrom ?? o.items_from) != null
          ? String(o.itemsFrom ?? o.items_from)
          : undefined,
      itemAs:
        (o.itemAs ?? o.item_as) != null
          ? String(o.itemAs ?? o.item_as)
          : undefined,
      itemKeyFrom:
        (o.itemKeyFrom ?? o.item_key_from) != null
          ? String(o.itemKeyFrom ?? o.item_key_from)
          : undefined,
      body: kind === "foreach" ? parsePlan(rawBody) : undefined,
      emitEvent:
        (o.emitEvent ?? o.emit_event ?? o.event) != null
          ? String(o.emitEvent ?? o.emit_event ?? o.event)
          : undefined,
      emitPayloadFrom:
        (o.emitPayloadFrom ?? o.emit_payload_from ?? o.payload_from) != null
          ? String(o.emitPayloadFrom ?? o.emit_payload_from ?? o.payload_from)
          : undefined,
      emitPayload: (() => {
...
export function validatePlan(
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    if (s.stepId && !/^[A-Za-z_$][A-Za-z0-9_$-]*$/.test(s.stepId))
      errors.push(
        `${at}: stepId must match [A-Za-z_$][A-Za-z0-9_$-]* so named results remain addressable`,
      );
    if (seen.has(s.stepId)) errors.push(`${at}: duplicate stepId`);
    if (!PLAN_KINDS.includes(s.kind))
      errors.push(`${at}: invalid kind "${s.kind}"`);
    if (
      s.timeoutS !== undefined &&
      (!Number.isInteger(s.timeoutS) || s.timeoutS <= 0)
    ) {
      errors.push(
        `${at}: timeoutS must be a positive integer number of seconds`,
      );
    }

    const sideEffecting = s.kind === "tool" || s.kind === "invoke";
    if (s.kind === "tool" && !s.tool)
      errors.push(`${at}: tool step needs a "tool" name`);
    if (s.kind !== "tool" && s.toolArguments !== undefined)
      errors.push(`${at}: toolArguments is only valid for tool steps`);
    if (s.kind !== "tool" && s.resultMap !== undefined)
      errors.push(`${at}: resultMap is only valid for tool steps`);
    if (s.kind === "tool" && s.toolArguments === undefined) {
      warnings.push(
        `${at}: legacy whole-carry tool invocation (toolArguments is absent)`,
      );
    }
    if (s.toolArguments !== undefined) {
      ...
      if (!entries.length)
        errors.push(
          `${at}: toolArguments must contain at least one explicit argument`,
        );
      for (const [argument, template] of entries) {
        if (
          !/^[A-Za-z_$][A-Za-z0-9_$-]*$/.test(argument) ||
          ["__proto__", "prototype", "constructor"].includes(argument)
        ) {
          errors.push(
            `${at}: toolArguments key "${argument}" is not a safe argument name`,
          );
          continue;
        }
        if (
          ...
          errors.push(
            `${at}: toolArguments.${argument} must be {from,...} or {const:...}; bare values are forbidden`,
          );
          continue;
        }
        const value = template as unknown as Record<string, unknown>;
        const hasFrom = Object.prototype.hasOwnProperty.call(value, "from");
        const hasFromFirst = Object.prototype.hasOwnProperty.call(
          value,
          "fromFirst",
        );
        const hasConst = Object.prototype.hasOwnProperty.call(value, "const");
        if (
          [hasFrom, hasFromFirst, hasConst].filter(Boolean).length !== 1 ||
          Object.keys(value).some(
            (key) => !["from", "fromFirst", "required", "const"].includes(key),
          )
        ) {
          errors.push(
            `${at}: toolArguments.${argument} must choose exactly one of {from}, {fromFirst} or {const}`,
          );
          continue;
        }
        if (hasFrom || hasFromFirst) {
          ...
          const candidates = hasFrom
            ...
            : Array.isArray(value.fromFirst)
              ? value.fromFirst
              : [];
          if (hasFromFirst && candidates.length < 2) {
            errors.push(
              `${at}: toolArguments.${argument}.fromFirst needs at least two candidates — with one it is just {from}`,
            );
          }
          for (const candidate of candidates) {
            const path = typeof candidate === "string" ? candidate : "";
            if (
              !VALUE_PATH_RE.test(path) ||
              !/^(event(?:\.|$)|input(?:\.|$)|lastResult(?:\.|$)|results(?:\.|$)|locals(?:\.|$))/.test(
                path,
              )
            ) {
              errors.push(
                `${at}: toolArguments.${argument} must be a safe path rooted at event/input/lastResult/results/locals`,
              );
            }
          }
          if (
            ...
            errors.push(
              `${at}: toolArguments.${argument}.required must be boolean`,
            );
          }
        } else if (!isSafePlanJson(value.const)) {
          errors.push(
            `${at}: toolArguments.${argument}.const must be safe finite JSON`,
          );
        }
      }
    }
    if (s.resultMap !== undefined) {
      ...
      if (
        ...
        errors.push(
          `${at}: resultMap.fields must contain at least one named field`,
        );
      } else {
        for (const [field, path] of Object.entries(fields)) {
          if (
            !/^[A-Za-z_$][A-Za-z0-9_$-]*$/.test(field) ||
            ["__proto__", "prototype", "constructor", "_raw"].includes(field)
          ) {
            errors.push(
              `${at}: resultMap field "${field}" is not a safe named field`,
            );
          }
          ...
          if (typeof path === "string") {
            if (!VALUE_PATH_RE.test(path) || !/^result(?:\.|$)/.test(path)) {
              errors.push(
                `${at}: resultMap.${field} must be a safe path rooted at result`,
              );
            }
          } else if (!path || typeof path !== "object" || Array.isArray(path)) {
            errors.push(
              `${at}: resultMap.${field} must be a path string or a {from,...} descriptor`,
            );
          } else {
            ...
            if (Object.keys(descriptor).some((key) => !allowed.includes(key))) {
              errors.push(
                `${at}: resultMap.${field} has an unknown key (allowed: ${allowed.join(", ")})`,
              );
            }
            const paths = [
              descriptor.from,
              ...(Array.isArray(descriptor.fallbackFrom)
                ? descriptor.fallbackFrom
                : []),
            ];
            ...
            for (const key of ["default", "fallbackConst"] as const) {
              if (
                ...
                !isSafePlanJson(descriptor[key])
              ) {
                errors.push(
                  `${at}: resultMap.${field}.${key} must be safe finite JSON`,
                );
              }
            }
          }
      ...
      if (
        s.resultMap.includeRaw !== undefined &&
        typeof s.resultMap.includeRaw !== "boolean"
      ) {
        errors.push(`${at}: resultMap.includeRaw must be boolean`);
      }
    }
    if (s.kind === "invoke" && !s.invoke)
      errors.push(`${at}: invoke step needs an "invoke" target`);
    // #EMIT-BRANCH — a branch that exists only in prose is not a branch. A
    // non-condition step may now carry its own executable condition guard; if
    // it does not, two distinct emits hanging off one logic predecessor still
    // fire every run. Refuse only the unguarded shape. Ontology-authored emit
    // guards remain on the emit steps and are executed by every renderer/runtime.
    if (s.kind === "emit" && s.emitEvent) {
      for (const dep of s.dependsOn ?? []) {
        const predecessor = plan.find((other) => other.stepId === dep);
        if (!predecessor || predecessor.kind === "condition") continue;
        const siblings = plan.filter(
          (other) =>
            other.kind === "emit" &&
            other.stepId !== s.stepId &&
            (other.dependsOn ?? []).includes(dep) &&
            other.emitEvent &&
            other.emitEvent !== s.emitEvent,
        );
        const branchIsExecutable =
          Boolean(s.condition) &&
          siblings.every((sibling) => sibling.condition);
        if (siblings.length > 0 && !branchIsExecutable) {
          errors.push(
            `${at}: emits "${s.emitEvent}" while ${siblings
              .map((other) => `"${other.emitEvent}"`)
              .join(
                ", ",
              )} also depend on "${dep}" with no condition — express the branch as a condition step with routes, or both events fire on every run`,
          );
        }
      }
    }
    if (s.kind === "condition" && !s.condition)
      errors.push(`${at}: condition step needs a "condition" expression`);
    if (s.kind !== "condition" && s.routes) {
      errors.push(`${at}: routes is only valid for condition steps`);
    }
    if (s.kind === "emit" && !s.emitEvent)
      errors.push(`${at}: emit step needs an "emitEvent"`);
    if (s.emitPayloadFrom && !VALUE_PATH_RE.test(s.emitPayloadFrom))
      errors.push(`${at}: emitPayloadFrom is not a safe data path`);
    if (
      s.kind === "emit" &&
      s.emitEvent &&
      opts?.declaredEvents &&
      !opts.declaredEvents.includes(s.emitEvent)
    ) {
      errors.push(
        `${at}: emitEvent "${s.emitEvent}" is not in the agent's declared emit allow-list`,
      );
    }
    if (s.kind === "foreach") {
      if (!s.itemsFrom)
        errors.push(`${at}: foreach step needs an "itemsFrom" path`);
      if (!s.itemKeyFrom)
        errors.push(
          `${at}: foreach step needs itemKeyFrom (a replay-stable item key)`,
        );
      if (s.itemsFrom && !VALUE_PATH_RE.test(s.itemsFrom))
        errors.push(`${at}: itemsFrom is not a safe data path`);
      if (s.itemKeyFrom && !VALUE_PATH_RE.test(s.itemKeyFrom))
        errors.push(`${at}: itemKeyFrom is not a safe data path`);
      if (!s.body?.length)
        errors.push(`${at}: foreach step needs a non-empty body`);
      if (s.itemAs && !/^[A-Za-z_$][A-Za-z0-9_$-]*$/.test(s.itemAs))
        errors.push(`${at}: itemAs must be an addressable local name`);
      const nested = s.body ?? [];
      const nestedValidation = validatePlan(nested, opts, {
        insideForeach: true,
      });
      for (const error of nestedValidation.errors)
        errors.push(`${at} body: ${error}`);
      for (const warning of nestedValidation.warnings)
        warnings.push(`${at} body: ${warning}`);
    }
    if (s.condition) {
      const syntaxError = validateConditionSyntax(s.condition);
      if (syntaxError) errors.push(`${at}: invalid condition — ${syntaxError}`);
      for (const match of s.condition.matchAll(
        /\bresults\.([A-Za-z_$][A-Za-z0-9_$-]*)/g,
      )) {
        if (!seen.has(match[1]!))
          errors.push(
            `${at}: condition references unknown/forward result "${match[1]}"`,
          );
      }
    }
    ...
    if (s.kind === "condition" && s.routes) {
      for (const [side, event] of Object.entries(s.routes)) {
        if (!event)
          errors.push(`${at}: routes.${side} needs a declared event name`);
        else if (opts?.declaredEvents && !opts.declaredEvents.includes(event)) {
          errors.push(
            `${at}: routes.${side} "${event}" is not in the agent's declared emit allow-list`,
          );
        }
      }
    }
    if (s.kind === "condition" && !s.routes) {
      const readLater = plan.some(
        (other, j) => j > i && (other.dependsOn ?? []).includes(s.stepId),
      );
      if (!readLater) {
        warnings.push(
          `${at}: condition result is never read — no later step depends on it and it declares no routes, so this routing rule has no effect`,
        );
      }
    }
    if (
      ...
      s.tool &&
      opts?.knownTools &&
      !opts.knownTools.includes(s.tool)
    ) {
      errors.push(`${at}: tool "${s.tool}" is not in the resolved tool set`);
    }
    if (sideEffecting && !s.idempotencyKeyFrom && !context?.insideForeach)
      errors.push(
        `${at}: side-effecting step needs idempotencyKeyFrom (a replay-stable business key)`,
      );
    if (sideEffecting && !s.onError && !s.errorPolicy?.length)
      errors.push(
        `${at}: side-effecting step needs an onError/errorPolicy failure policy`,
      );
    if (s.onError && s.errorPolicy?.length)
      errors.push(`${at}: choose legacy onError or errorPolicy, not both`);
    if (
      s.kind === "invoke" &&
      s.onError === "soft" &&
      s.defaultResult === undefined
    ) {
      errors.push(
        `${at}: invoke onError=soft requires an explicit defaultResult`,
      );
    }
    if (s.errorPolicy?.length) {
      const defaults = s.errorPolicy.flatMap((rule, index) =>
        "default" in rule ? [index] : [],
      );
      if (defaults.length !== 1)
        errors.push(`${at}: errorPolicy requires exactly one default rule`);
      else if (defaults[0] !== s.errorPolicy.length - 1)
        errors.push(`${at}: errorPolicy default rule must be last`);
      for (let ruleIndex = 0; ruleIndex < s.errorPolicy.length; ruleIndex++) {
        ...
        if ("when" in rule) {
          ...
          if (predicateError)
            errors.push(
              `${at}: errorPolicy rule #${ruleIndex + 1}: ${predicateError}`,
            );
        }
        if (
          outcome === "continue" &&
          rule.defaultResult === undefined &&
          s.defaultResult === undefined
        ) {
          errors.push(
            `${at}: errorPolicy continue rule #${ruleIndex + 1} requires defaultResult`,
          );
        }
        if (
          rule.emitEvent &&
          opts?.declaredEvents &&
          !opts.declaredEvents.includes(rule.emitEvent)
        ) {
          errors.push(
            `${at}: errorPolicy emitEvent "${rule.emitEvent}" is not in the agent's declared emit allow-list`,
          );
        }
        if (rule.emitEvent && rule.suppressEmit) {
          errors.push(
            `${at}: errorPolicy rule #${ruleIndex + 1} cannot both emitEvent and suppressEmit`,
          );
        }
      }
    }
    ...
    for (const dep of s.dependsOn ?? []) {
      if (!seen.has(dep))
        errors.push(
          `${at}: dependsOn "${dep}" is not a prior step (forward/unknown reference)`,
        );
    }
    seen.add(s.stepId);
  }
...
export type ManifestAction = Record<string, unknown> & {
  order: string;
  name: string;
  type: string;
};
...
export function projectInputBinding(
  binding: GeneratedInputBinding,
): Record<string, unknown> {
  const common = {
    field: binding.field,
    type: binding.type,
  ...
  switch (binding.kind) {
    case "event":
      return {
        ...
        ...(binding.sourceEvents?.length
          ? { source_events: binding.sourceEvents }
          : {}),
    ...
    case "step_output":
      return {
        ...common,
        source_step: binding.sourceStep,
        source_output: binding.sourceOutput,
      };
  }
}

function orderedAcquisitionBindings(
  ...
  const pending = new Map(
    bindings
      .filter(
        (binding) =>
          binding.kind === "object_lookup" || binding.kind === "human_input",
      )
  ...
  while (pending.size) {
    const ready = [...pending.values()]
      .filter(
        ({ binding }) =>
          binding.kind !== "object_lookup" ||
          (binding.dependsOn ?? []).every((field) => !pending.has(field)),
      )
      .sort((left, right) => left.index - right.index)[0];
    if (!ready) {
      throw new Error(
        `input binding acquisition dependency cycle: ${[...pending.keys()].join(" -> ")}`,
      );
    }
...
function actionForStep(step: PlanStep, order: number): ManifestAction {
  const actionName =
    step.kind === "tool" ? step.tool || step.stepId : step.stepId;
  ...
  if (step.toolArguments) {
    a.tool_arguments = Object.fromEntries(
      Object.entries(step.toolArguments).map(([name, value]) => [
        name,
        "from" in value
          ? {
              from: value.from,
  ...
  if (step.resultMap) {
    a.result_map = {
      ...
      fields: Object.fromEntries(
        Object.entries(step.resultMap.fields).map(([field, value]) => [
          field,
          typeof value === "string"
            ? value
            : {
                ...
                ...(value.default !== undefined
                  ...
                  : {}),
                ...(value.fallbackFrom?.length
                  ? { fallback_from: value.fallbackFrom }
                  : {}),
                ...(value.fallbackConst !== undefined
                  ? { fallback_const: value.fallbackConst }
                  : {}),
              },
        ]),
      ),
  ...
  if (step.onEmpty) {
    a.on_empty =
      "emitEvent" in step.onEmpty
        ? {
            emit_event: step.onEmpty.emitEvent,
            ...(step.onEmpty.emitPayload
              ? { emit_payload: step.onEmpty.emitPayload }
              : {}),
          }
  ...
  if (step.errorPolicy?.length) {
    a.on_error = step.errorPolicy.map((rule) => ({
      ...(Object.prototype.hasOwnProperty.call(rule, "when")
        ? {
            when: (rule as Extract<ErrorPolicyRule, { when: string }>).when,
            do: (rule as Extract<ErrorPolicyRule, { when: string }>).do,
          }
        : {
            default: (
              rule as Extract<ErrorPolicyRule, { default: ErrorPolicyAction }>
            ).default,
          }),
      ...(Object.prototype.hasOwnProperty.call(rule, "defaultResult")
        ? { default_result: rule.defaultResult }
        : {}),
      ...(rule.emitEvent ? { emit_event: rule.emitEvent } : {}),
      ...(rule.emitPayload ? { emit_payload: rule.emitPayload } : {}),
      ...(rule.suppressEmit !== undefined
        ? { suppress_emit: rule.suppressEmit }
        : {}),
    }));
  ...
  if (step.condition) {
    // `condition` is a step-local fail-closed guard for every action type.
    // kind:"condition" additionally records a branch verdict/routes; emit,
    // tool, invoke and logic conditions gate the side effect itself.
    a.condition = step.condition;
  } else if (step.kind === "condition") {
    a.condition = "true";
  }
  if (step.kind === "condition") {
    if (step.routes)
      a.routes = { on_true: step.routes.onTrue, on_false: step.routes.onFalse };
  }
  if (step.kind === "invoke") {
  ...
  if (step.kind === "foreach") {
    ...
    a.foreach_actions = (step.body ?? []).map((child, index) =>
      actionForStep(child, index + 1),
    );
  }
  if (step.kind === "emit") {
  ...
  const walk = (steps: PlanStep[]): void => {
    for (const step of steps) {
      for (const rule of step.errorPolicy ?? []) {
        if (rule.compensate)
          found.push(`${step.stepId}.errorPolicy.compensate`);
      }
      if (step.healthSignals?.length)
        found.push(`${step.stepId}.healthSignals`);
...
export function projectPlanToActions(
  ...
  for (const [index, binding] of orderedAcquisitionBindings(
    spec.inputBindings ?? [],
  ).entries()) {
  ...
  if (spec.codeExecuted === true) {
    if (spec.decisionTables?.length) {
      throw new Error(
        `codeExecuted agent "${spec.slug}" cannot also project decisionTables; choose one execution owner`,
      );
    }
    actions.push({
      order: String(order++),
      ...
      description:
        (spec.designReasoning ?? "").slice(0, 200) ||
        spec.actionName ||
        spec.slug,
      type: "logic",
      allowed_tools: [],
