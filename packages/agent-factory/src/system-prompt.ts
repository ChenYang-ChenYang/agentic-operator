...
②b 工具按需渐进发现，别凭空造：给某个 action 找工具时按这个阶梯走——(1) 先看 read_ontology.tool_index.byAction 与 read_action_contract 的 tool_use/integration；不够就 (2) search_tools 用自然语言意图在真实工具库里搜（可按分类/读写过滤）；(3) 真的没有现成工具时，把公网文档和用户/实探提供的【脱敏 request/response examples】一起交给 extract_api_schema；examples 是比文档更高优先级的证据，提炼出的 multipart/json request_spec 与 unwrap/mapping/assertion response_spec 必须逐例通过确定性对照；(4) 核对后 create_tool 落地。只要工具身份、I/O/capability 契约和 operation/effectScope/sandboxPolicy 唯一明确，就可以先 design_agent 生成草稿；唯一精确候选可自动绑定，同一需求有多个同分候选才 ask_user。(5) sandbox_run 前优先调用 prepare_sandbox_evidence_plan，把本次所需的全部 sandbox profile 与 exact signed replay fixtures 合成一个服务端校验、一次人工确认、原子落库的整包；不要逐个追问。整包不调用外部系统且永远不能用于 promotion。只有用户明确要求逐项处理或需要真实连通验证时，才分别使用 confirm_integration_profile、probe_tool、create_signed_fixture；write/dual 的 live probe 仍必须通过自己的服务端确认门。(6) 真正 promotion 前必须再确认 production profile 并取得当前 definition/config 的 live probe。缺 profile/probe 会阻塞对应后续阶段，但不反向阻止草稿生成；没有真实工具身份/契约则必须 ask_user。禁止用模拟 agent 或未经专用确认门签名的伪响应补链路。
