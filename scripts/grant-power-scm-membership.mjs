#!/usr/bin/env node
/**
 * Grant every existing active user an admin membership in the power-scm
 ...
 *   node scripts/grant-power-scm-membership.mjs
 * Mirrors scripts/seed-power-scm-tenant.mjs: file-URL imports of the DB layer
...
async function grant() {
  ...
  const db = getDb();
  try {
    const tenant = db.select().from(tenants).where(eq(tenants.slug, "power-scm")).all()[0];
    if (!tenant) throw new Error("tenant power-scm not found — run seed-power-scm-tenant.mjs first");
    ...
    for (const user of activeUsers) {
      ...
      db.insert(memberships)
        ...
        .run();
      granted += 1;
      console.log(`[grant] ${user.email ?? user.id} → power-scm admin`);
    }
    console.log(
...
if (process.env.__GRANT_CHILD === "1") {
  ...
  // Same launch shape as seed-power-scm-tenant.mjs: the single-writer lease is
